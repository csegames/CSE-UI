...
export class ExternalDataSource<P = {}, S = {}, SS = any> extends React.Component<P, S, SS> {
  ...
  protected onReduxUpdate: (() => void) | null = null;
  ...
  getActiveCharacter(): string | undefined {
    return getCharacterID(game.accessToken);
  }
  ...
  protected rebind(handles: ListenerHandle[]): void {
    ...
    if (this.onReduxUpdate) {
      const unsub = store.subscribe(this.onReduxUpdate);
      this.handles.push({ close: unsub });
    }
  }
}
