...
export class FormattedTextDiv extends React.Component<Props> {
  ...
  public render() {
    const { text, textClasses, textAlign, nodes, className, children, ...otherProps } = this.props;
  ...
  private renderWords(): React.ReactNode[] {
    ...
    lines.forEach((line, lineIndex) => {
          ...
          if (orderedTagData[tagIndex]) {
            ...
            switch (data.tag) {
              ...
              case 'n': {
                ...
                subSpans.push(this.props.nodes?.[data.tagIndex] ?? null);
                break;
              }
            }
          }
      ...
      spans.push(
        <div className={`${Line} ${this.props.textAlign ?? 'left'}`} key={`Line${lineIndex}`}>
          {lineSpans}
        </div>
      );
    });
