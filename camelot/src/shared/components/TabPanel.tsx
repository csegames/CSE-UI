...
export class TabPanel<TabData> extends React.Component<TabPanelProps<TabData>, TabPanelState> {
  ...
  private renderTabs(customStyle: Partial<TabPanelStyle>): JSX.Element {
    return (
      <div className={`${Tabs} ${customStyle.tabs}`}>
          ...
          if (typeof this.props.renderTabDivider === 'function') {
            return [
              <div
                ...
                onClick={this.selectIndex.bind(this, index, tabItem.name, tabItem.shouldTriggerNavEvent)}
              >
  ...
  private renderAllContent(customStyles: Partial<TabPanelStyle>): JSX.Element[] {
  ...
  private renderActiveContent(customStyles: Partial<TabPanelStyle>): JSX.Element {
    const activeItem: ContentItem = this.props.content.find((value) => {
      return this.props.tabs[this.activeTabIndex].rendersContent === value.name;
    });
  ...
  private selectIndex(index: number, name: string, shouldTriggerNavEvent: boolean): void {
