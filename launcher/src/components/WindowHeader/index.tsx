...
class WindowHeader extends React.Component<WindowHeaderProps, WindowHeaderState> {
  ...
  public render() {
    const soundMuteIcon = this.props.soundsState.playSound ? (
      <img src='images/mute-fx.png' />
    ) : (
      <img src='images/muted-fx.png' />
    );
    const musicMuteIcon = this.props.soundsState.playMusic ? (
    ...
    if (this.state.settingsOpen)
      settings = (
        <h1 style={{ color: '#fff' }} onClick={this.closeSettings}>
          Settings!
        </h1>
      );
    return (
