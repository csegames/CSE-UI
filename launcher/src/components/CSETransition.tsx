...
import { SimpleRect, simpleRectFromDOMRect } from '../lib/SimpleBounds';
...
class ACSETransition extends React.Component<Props, State> {
  ...
  public render(): React.ReactNode {
    const { vminPx, children, className, style, ...otherProps } = this.props;
  ...
  getBoundingClientRect(): SimpleRect {
    return (
      simpleRectFromDOMRect(this.rootRef?.getBoundingClientRect()) ?? {
        x: 0,
        y: 0,
        width: 0,
        height: 0,
  ...
  private applyTransitionState(newState: TransitionState): void {
    ...
    switch (newState) {
      ...
      case TransitionState.Exiting: {
        ...
        }, (this.props.exitDurationMS ?? DefaultExitAnimationDurationMS) - 16);
        break;
      }
...
export const CSETransition = connect(mapStateToProps)(ACSETransition);
