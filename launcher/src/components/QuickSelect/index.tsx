...
class QuickSelect extends React.Component<QuickSelectProps, QuickSelectState> {
  ...
  public render() {
    if (this.props.items.length === 0) {
      ...
      return (
        <div className={`QuickSelect ${this.props.containerClass || ''}`}>
          <div className='QuickSelect__activeView'>{this.props.activeViewComponentGenerator(null)}</div>
          <div className='QuickSelect__arrow'>
    ...
    const selectedIndex =
      this.props.selectedItemIndex !== undefined ? this.props.selectedItemIndex : this.state.selectedIndex;
    return (
      <div
      ...
      >
        ...
        />
        <div
          className='QuickSelect__activeView'
          onClick={(e) => {
        ...
        <div
          className='QuickSelect__arrow'
          onClick={(e) => {
            this.showList(!this.state.showList);
            e.stopPropagation();
          }}
        >
          <i className={`fa ${this.state.showList ? 'fa-chevron-down' : 'fa-chevron-up'}`} aria-hidden='true'></i>
        </div>
        <div
          className={`QuickSelect__listView ${this.state.showList ? '' : 'QuickSelect__listView--hidden'}`}
  ...
  private showList = (visible: boolean) => {
    if (visible) playSound(Sound.SelectChange);
