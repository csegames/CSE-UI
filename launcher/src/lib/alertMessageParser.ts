...
type AlertMessageVar = { key: string; value: string };
type AlertMessageFormatterFunc = (value: any) => string;
type AlertMessageFormatters = { [key: string]: AlertMessageFormatterFunc };
...
function formatTimeDeltaForAlertMessage(totalSeconds: number): string {
  if (totalSeconds < 0) {
...
function formatDateForAlertMessage(date: Date): string {
