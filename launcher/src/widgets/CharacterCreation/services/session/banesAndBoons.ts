...
type Trait = shard.Trait;
const TraitCategory = shard.TraitCategory;
type TraitsInfo = shard.TraitsInfo;
...
type TraitsResult = Pick<shard.CUQuery, 'traits'>;

const traitsQuery = gql`
  query TraitsQuery {
    traits {
      maxAllowed
      minRequired
...
async function getTraits(dispatch: (action: any) => any, payload: FetchTraitInterface) {
  const res = await query<TraitsResult>({ query: traitsQuery }, shardConf(payload.apiHost));
  if (res.ok) {
...
export const resetBaneOrBoon = (payload: FetchTraitInterface) => {
  return (dispatch: (action: any) => any) => {
    return webAPI.TraitsAPI.GetTraitsV1(shardConf(payload.apiHost)).then((result) => {
...
export const onSelectRankBoon = module.createAction({
  ...
  reducer: (state, action) => {
    ...
    const nextRankBoon = action.boon.ranks[action.boon.rank + 1]
      ? action.boon.ranks[action.boon.rank + 1]
      : action.boon.ranks[action.boon.rank];
    const previousRankBoon = action.boon.ranks[action.boon.rank - 1]
      ? action.boon.ranks[action.boon.rank - 1]
      : action.boon.ranks[action.boon.rank];
...
export const onSelectRankBane = module.createAction({
  ...
  reducer: (state, action) => {
    ...
    const nextRankBane = action.bane.ranks[action.bane.rank + 1]
      ? action.bane.ranks[action.bane.rank + 1]
      : action.bane.id;
    const previousRankBane = action.bane.ranks[action.bane.rank - 1]
      ? action.bane.ranks[action.bane.rank - 1]
      : action.bane.id;
...
export const onInitializeTraits = module.createAction({
  ...
  reducer: (state, action) => {
    ...
    const combinedRanksArr =
      allRanks.length > 0
        ? allRanks.reduce((rank1: BanesAndBoonsInfo[], rank2: BanesAndBoonsInfo[]) => rank1.concat(rank2))
        : [];
    ...
    allTraits.forEach((trait: Trait, index: number) => {
      if (!trait.exclusives || !trait.exclusives.ids || trait.exclusives.ids.length == 0) {
        return;
      }

      trait.exclusives.ids.forEach((exclusive: string) => {
        traits[exclusive] = Object.assign({}, traits[exclusive], {
          exclusivityGroup: trait.exclusives.ids,
          minRequired: trait.exclusives.minRequired,
          maxAllowed: trait.exclusives.maxAllowed
        });
    ...
    Object.keys(playerClasses).forEach((playerClass) => {
      if (playerClass !== 'Any') {
        allClassTraits.push({
          id: playerClass,
          optional:
            playerClasses[playerClass] && playerClasses[playerClass].optional
              ? playerClasses[playerClass].optional
              : [],
          required:
            playerClasses[playerClass] && playerClasses[playerClass].required ? playerClasses[playerClass].required : []
        });
      }
    });
    ...
    // Filter traits according to selected class, faction, and race.
    ...
    const requiredBoons = [
      ...(playerClassTraits && playerClassTraits.required
        ? playerClassTraits.required.filter((id: string) => traits[id].isBoon)
        : []),
      ...(factionTraits && factionTraits.required
        ? factionTraits.required.filter((id: string) => traits[id].isBoon)
        : []),
      ...(raceTraits && raceTraits.required ? raceTraits.required.filter((id: string) => traits[id].isBoon) : [])
    ].map((id) => {
    ...
    const mappedClassTraits =
      _.reduce(
        allClassTraits.map((playerClass) => [...playerClass.optional, ...playerClass.required]),
        (a: string[], b: string[]) => a.concat(b)
      ) || [];
    const mappedRaceTraits =
      _.reduce(
        allRaceTraits.map((race) => [...race.optional, ...race.required]),
        (a: string[], b: string[]) => a.concat(b)
      ) || [];
    const mappedFactionTraits =
      _.reduce(
        allFactionTraits.map((faction) => [...faction.optional, ...faction.required]),
        (a: string[], b: string[]) => a.concat(b)
      ) || [];
    ...
    if (initType === 'boons' || initType === 'both') {
      // playerClassBoons
      playerClassTraits &&
        playerClassTraits.optional &&
        [
          ...(playerClassTraits.optional.filter((id: string) => {
            traits[id] = combinedRanks[id] ? { ...traits[id], category: TraitCategory.Class } : traits[id];
            return traits[id].isBoon && !combinedRanks[id];
          }) || []),
          ...(playerClassTraits.required.filter((id: string) => {
            traits[id] = combinedRanks[id]
              ? { ...traits[id], category: TraitCategory.Class }
              : { ...traits[id], selected: true };
            return traits[id].isBoon && !combinedRanks[id];
          }) || []),
          playerClassTraits.optional.find((id: string) => firstRanks[id] && traits[id].isBoon)
        ]
          .filter((id: string) => id)
          .forEach((key: string) => {
      ...
      factionTraits &&
        ...
        [
          ...
          ...(factionTraits.required.filter((id: string) => {
            traits[id] = combinedRanks[id]
              ? { ...traits[id], category: TraitCategory.Faction, selected: true }
              : { ...traits[id], selected: true };
            return traits[id].isBoon && !combinedRanks[id];
          }) || []),
          factionTraits.optional.find((id: string) => firstRanks[id] && traits[id].isBoon)
        ]
          .filter((id: string) => id)
          .forEach((key: string) => {
      ...
      raceTraits &&
        ...
        [
          ...
          ...(raceTraits.required.filter((id: string) => {
            traits[id] = combinedRanks[id]
              ? { ...traits[id], category: TraitCategory.Race, selected: true }
              : { ...traits[id], selected: true };
            return traits[id].isBoon && !combinedRanks[id];
          }) || []),
          raceTraits.optional.find((id: string) => firstRanks[id] && traits[id].isBoon)
        ]
          .filter((id: string) => id)
          .forEach((key: string) => {
            raceBoons[key] = key;
        ...
        generalTraits.optional &&
          [
            ...
            ...(generalTraits.required.filter((id: string) => {
              traits[id] = combinedRanks[id]
                ? { ...traits[id], category: TraitCategory.General, selected: true }
                : { ...traits[id], selected: true };
              return traits[id].isBoon && !combinedRanks[id];
            }) || []),
            generalTraits.optional.find((id: string) => firstRanks[id] && traits[id].isBoon)
          ]
            .filter((id: string) => id)
            .forEach((key: string) => {
    ...
    if (initType === 'banes' || initType === 'both') {
      ...
      playerClassTraits &&
        ...
        [
          ...
          ...(playerClassTraits.required.filter((id: string) => {
            traits[id] = combinedRanks[id]
              ? { ...traits[id], category: TraitCategory.Class, selected: true }
              : { ...traits[id], selected: true };
            return !traits[id].isBoon && !combinedRanks[id];
          }) || []),
          playerClassTraits.optional.find((id: string) => firstRanks[id] && !traits[id].isBoon)
    ...
    // Total points
    const totalPoints =
      (requiredBoons.length > 0 && requiredBoons.map((boon) => boon.points).reduce((a, b) => a + b)) +
    ...
    allTraitsWithPrerequisites
      .filter((t: BanesAndBoonsInfo) => t.prerequisites.filter((preReq) => traits[preReq]).length !== 0)
      .forEach((boon: BanesAndBoonsInfo) =>
        boon.prerequisites.forEach((preReq) => (allPrerequisites[preReq] = preReq))
      );
...
export const onResetBoons = module.createAction({
  ...
  action: (action: { banesAndBoons: any; playerClass: string; faction: string; race: string }) => action,
  reducer: (state, action) => {
    ...
    const allClassTraits = Object.keys(playerClasses).map((playerClass) => {
      return {
        optional:
          playerClasses[playerClass] && playerClasses[playerClass].optional ? playerClasses[playerClass].optional : [],
        required:
          playerClasses[playerClass] && playerClasses[playerClass].required ? playerClasses[playerClass].required : []
      };
    });
    ...
    const requiredBoons = [
      ...(generalTraits && generalTraits.required
        ? generalTraits.required.filter((id: string) => traits[id].isBoon)
        : []),
      ...(playerClassTraits && playerClassTraits.required
        ? playerClassTraits.required.filter((id: string) => traits[id].isBoon)
        : []),
    ...
    const totalPoints =
      (Object.keys(state.addedBanes).length > 0 &&
        Object.keys(state.addedBanes)
          .map((id) => traits[id].points)
          .reduce((a, b) => a + b)) +
      (Object.keys(addedBoons).length > 0 &&
        Object.keys(addedBoons)
          .map((id) => traits[id].points)
          .reduce((a, b) => a + b));
    ...
    allBoonIds.forEach(
      (id: string) => (traits[id] = !traits[id].required ? { ...traits[id], selected: false } : traits[id])
    );
    return {
      ...state,
      generalBoons,
...
export const onResetBanes = module.createAction({
  ...
  reducer: (state, action) => {
    ...
    const totalPoints =
      (Object.keys(addedBanes).length > 0 &&
        Object.keys(addedBanes)
          .map((id) => traits[id].points)
          .reduce((a, b) => a + b)) +
      (Object.keys(state.addedBoons).length > 0 &&
        Object.keys(state.addedBoons)
          .map((id) => traits[id].points)
          .reduce((a, b) => a + b));
