...
const SelectedShadow = css`
  box-shadow: inset 0 -20px 30px rgba(244, 192, 102, 0.3);
`;
...
class Trait extends React.Component<TraitProps, {}> {
  ...
  public render() {
    ...
    const shouldBeDisabledBecausePreReqs =
      preReqs && preReqs.filter((preReq: string) => addedTraits[preReq]).length !== preReqs.length;

    const addedRankTrait =
      trait.ranks && (addedTraits[trait.ranks[trait.rank - 1]] || addedTraits[trait.ranks[trait.rank]]);

    const exclusivityGroup = allExclusives[trait.id] ? trait.exclusives.ids : [];

    const shouldBeDisabledBecauseExclusives =
      exclusivityGroup &&
      exclusivityGroup.length > 0 &&
      exclusivityGroup.filter((exclusive: string) => addedTraits[exclusive] && !addedTraits[trait.id]).length >=
        trait.maxAllowed;

    const shouldBeDisabled =
      shouldBeDisabledBecausePreReqs ||
      shouldBeDisabledBecauseExclusives ||
      trait.required ||
    ...
    return (
          ...
          <div>
            ...
            {exclusivityGroup.length > 0 && (
              <div>
                <DependenciesContainer>
                  Exclusive group:{' '}
                  {exclusivityGroup.map((exclusive: string, i: number) => (
                    <DependencyText key={i} style={{ color: addedTraits[exclusive] ? colors.success : 'red' }}>
                      {traits[exclusive].name}
                      {exclusive !== exclusivityGroup[exclusivityGroup.length - 1] && ', '}
                    </DependencyText>
                  ))}
                </DependenciesContainer>
      ...
      >
        <TraitView
          ...
          onClick={
            shouldBeDisabled ? () => {} : trait.ranks ? this.onRankClick.bind(this) : this.onTraitClick.bind(this)
          }
          style={{
        ...
        >
          <PointsCircle>{type === 'Bane' ? trait.points * -1 : trait.points}</PointsCircle>
          {trait.ranks && (
            <RankText>
          ...
          {!shouldBeDisabled ? (
            <div
              className={cx(!shouldBeDefault ? Shadow : '', trait.selected && !shouldBeDefault ? SelectedShadow : '')}
            />
          ) : (
            <div
              className={cx(
  ...
  private onRankClick(e: React.MouseEvent<HTMLDivElement>) {
    const { trait, traits, addedTraits, onTraitClick, onCancelTrait, onSelectRankTrait, onCancelRankTrait } =
      this.props;
