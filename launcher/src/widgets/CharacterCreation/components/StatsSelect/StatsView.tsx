...
import { printWithSeparator } from '../../../../lib/stringUtils';
...
export class StatsView extends React.Component<Props, State> {
  ...
  public render() {
    return (
      <div className='row'>
        <SectionTitleContainer id={this.props.title}>{this.props.title}</SectionTitleContainer>
        <GridStats
              ...
              return (
                <Tooltip
                  ...
                  content={() =>
                    statInfo.statDef && (
                      <TooltipContent>
                        <div>
                          {statInfo.statDef.name} {this.getStatDisplayValueString(statInfo)}
                        </div>
                ...
                >
                  <StatsListItem id={statInfo.statDef.name} className={isOdd ? LightListItem : ''}>
                    ...
                    <StatText id={`${statInfo.statDef.name}-value`} className={statInfo.value != null ? StatValue : ''}>
                      {this.getStatDisplayValueString(statInfo)}
                    </StatText>
                  </StatsListItem>
                </Tooltip>
              );
  ...
  private getStatDisplayValueString(statInfo: StatObjectInfo): string {
    return statInfo.value != null ? printWithSeparator(parseFloat(statInfo.value.toFixed(2)), ',') : '';
  }
}
