...
class CharacterSelect extends React.Component<Props, CharacterSelectState> {
  ...
  public render() {
    ...
    const selectedCharacter =
      this.state.selectedCharacter || this.props.selectedCharacter || _.values(this.props.characters)[0];

    const servers: Dictionary<PatcherServer> = {};
    ...
    return (
      ...
      >
        <Header>
          <CloseButton onClick={this.onClose.bind(this)}>X</CloseButton>
        </Header>
        ...
        <ListContainer ref={(ref) => (this.listRef = ref)}>
          <CharacterSelectList
            ...
            onCharacterSelect={this.onSelectCharacter.bind(this)}
            onChooseCharacter={this.onChooseCharacter.bind(this)}
  ...
  public componentDidMount() {
    globalEvents.on('character-select-show-delete', this.toggleModal.bind(this));
    window.addEventListener('keydown', this.handleEscKey.bind(this));
  }

  public componentWillReceiveProps(nextProps: Props) {
    if (
      (!this.state.selectedCharacter && nextProps.selectedCharacter) ||
  ...
  private onChooseCharacter(character: SimpleCharacter, server: PatcherServer) {
    ...
    this.props.onUpdateState(ContentPhase.Camelot, { selectedCharacter: character, selectedServer: server });
  }
  ...
  private handleEscKey(e: KeyboardEvent) {
    if (e.key === 'Escape' && this.props.charSelectVisible) {
      this.onClose();
    }
  }
