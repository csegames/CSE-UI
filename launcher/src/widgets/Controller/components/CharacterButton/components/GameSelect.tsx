...
const GameMask = styled.div`
  ...
  z-index: 1;
  transition: left ease 0.4s, width ease 0.4s, filter ease 0.4s, -webkit-mask-size ease 0.1s;
...
class GameSelect extends React.Component<GameSelectProps, GameSelectState> {
  ...
  public render() {
    ...
    return (
      <div>
        {patcher.hasPermissions() && (
          <AccessLevelText style={{ opacity: this.state.isOpen ? 0 : 0.5 }}>
            Your Access Level: {permissionsString(patcher.getPermissions(serverType))}
          </AccessLevelText>
        )}
        <GameMask
        ...
        >
          ...
          <SelectedGame key={serverType} style={{ backgroundImage: `url(${serverTypeToIcon(serverType)})` }} />
        </GameMask>
        <PopupContainer
        ...
        >
          {serverTypes.map((type) =>
            type === this.props.serverType ? null : (
              <GameSelectItem key={type} type={type} onSelectServerType={this.onSelectServerType.bind(this)} />
            )
          )}
        </PopupContainer>
      </div>
  ...
  private getServerTypes() {
    ...
    for (let i = ServerType.CUGAME; i < ServerType.UNKNOWN; ++i) {
      ...
      for (const key in this.props.servers) {
        ...
        const serverType = this.props.servers[key].type;
        if (serverType === i && patcher.hasToolsAccess(serverType)) {
          anyOfType = true;
          break;
        }
      }
