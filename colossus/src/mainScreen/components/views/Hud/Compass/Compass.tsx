...
class ACompass extends React.Component<Props, State> {
  public name: string = 'Compass';

  constructor(props: Props) {
    super(props);
    this.state = {
      handle: clientAPI.startAnimation(this.animate.bind(this)),
      viewBearing: NaN;
    }
  }

  private animate(data: AnimationData, timestamp: DOMHighResTimeStamp) {
    
  }

  public render() {
    const facing = this.state.viewBearing;
    if (isNaN(facing))
    {
      return null;
    }
  ...
  private getCompassObjectives(): JSX.Element[] {
    ...
    for (let entityId in this.props.objectives) {
      const obj: BaseEntityState = this.props.objectives[entityId];
      ...
      if ((obj.objective.visibility & ObjectiveUIVisibility.Compass) !== 0) {
        if (!isItem(obj)) {
          console.error(
            'Compass.tsx assumes that Objectives are found only on ItemEntityState.  If that is no longer true, please fix it!'
          );
          continue;
        }
        const itemObjective: ItemEntityState = obj as ItemEntityState;
  ...
  private getBossIndicators(): JSX.Element[] {
    ...
    for (let entityID in this.props.bosses) {
      const boss: PlayerEntityState = this.props.bosses[entityID];
      if (!boss.iconClass) {
        continue;
      }
...
function mapStateToProps(state: RootState, ownProps: ComponentProps) {
  const { self, bosses, objectives, positions } = state.entities;
  return {
    primaryObjectiveDetails: state.game.objectiveDetailsPrimary,
    scenarioRoundStartTime: self.scenarioRoundStateStartTime,
    objectives,
    bosses,
