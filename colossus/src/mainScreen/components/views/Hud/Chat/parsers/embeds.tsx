...
function isVideo(text: string) {
  ...
  let vimeoURL: RegExpMatchArray = text.match(
    /^http[s]?:\/\/(?:www\.|player\.)?vimeo\.com\/(?:channels\/(?:\w+\/)?|groups\/(?:[^\/]*)\/videos\/|album\/(?:\d+)\/video\/|video\/|)(\d+)(?:$|\/|\?)/
  );
  let twitchURL: RegExpMatchArray = text.match(/^http[s]?:\/\/(?:www\.)?twitch\.tv\//);
  if (youtubeURL) {
    let youtubeMatch: RegExpMatchArray = text.match(
...
export function parseEmbeds(text: string, opts: ChatOptionsState, next: Parser): [string, JSX.Element] {
  ...
  if (isImage(url)) {
    ...
    return [
      ...
      <>
        ...
        <br />
        <a key={genID()} target="_blank" href={url}>
          <img onLoad={triggerAutoScroll} src={url} title={url} height={200} />
        </a>
      </>
    ];
  } else if (isGif(url)) {
    ...
    return [
      ...
      <>
        ...
        <a key={genID()} className="chat-line-message" target="_blank" href={url}>
          <iframe className="chat-line-video" src={video} allowFullScreen />
        </a>
      </>
    ];
