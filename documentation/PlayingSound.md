...
4. There is a function attached to the global `clientAPI` object called `playGameSound`. Whenever you want to play a sound, just call the function and pass in the sound ID to the parameter.

```
clientAPI.playGameSound(SoundEvents.PLAY_UI_MENU_BOONSELECT);
clientAPI.playGameSound(SoundEvents.PLAY_UI_MENU_BANESELECT);
etc.
```
