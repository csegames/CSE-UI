...
class SelectServer extends React.Component<SelectServerProps, SelectServerState> {
  ...
  renderList() {
    return (
      <div className='SelectServer__ServerList'>
  ...
  renderItem(item: any) {
    if (item.serverInfo) {
      ...
      return (
        <div onClick={() => this.onSelectedServerChanged(item)}>
  ...
  render() {
    ...
    return (
      <div className='SelectServer'> 
        <div className='SelectServer_ActiveItem' onClick={() => this.setState({showList: !this.state.showList} as any)}>
