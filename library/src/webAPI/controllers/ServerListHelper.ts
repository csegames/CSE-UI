...
export function getAvailableZones(shard: number) {
  return create(createOptions()).get('v1/availableZones', { 
    shard: shard
  });
}

export function getZoneInfo(shard: number, zoneID: string) {
  return create(createOptions()).get('v1/getZoneInfo', { 
    shard: shard, 
    zoneID: zoneID
  });
}
