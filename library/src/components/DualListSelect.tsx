 ...
 * @Last Modified time: 2017-02-17 17:58:54
 */
...
export class DualListSelect extends React.Component<DualListSelectProps, DualListSelectState> {
  ...
  onLeftFilterTextChanged = (e: React.ChangeEvent<HTMLInputElement>) => {
    this.filterLeftItems(e.target.value);
  }

  onRightFilterTextChanged = (e: React.ChangeEvent<HTMLInputElement>) => {
    this.filterRightItems(e.target.value);
  }

  // remove from left add to right
  selectItem = (selectKey?: string) => {
  ...
  filterRightItems = (rightFilter: string) => {
    if (rightFilter == '') {
      this.setState({
  ...
  filterLeftItems = (leftFilter: string) => {
    if (leftFilter == '') {
      this.setState({
  ...
  renderLeftItems = (ss: DualListSelectStyle, custom: Partial<DualListSelectStyle>) => {
  ...
  renderRightItems = (ss: DualListSelectStyle, custom: Partial<DualListSelectStyle>) => {
    var items: JSX.Element[] = [];
  ...
  leftInputRef: HTMLInputElement = null;
  rightInputRef: HTMLInputElement = null;
    ...
    return (
      <div className={css(ss.container, custom.container)}>
        ...
        <div className={css(ss.buttons, custom.buttons)}>
          <RaisedButton styles={{button: merge(defaultDualListSelectStyle.button, this.props.styles ? this.props.styles.button : null || {})}}
