...
import { Client, Element, xmpp } from 'node-xmpp-client';
...
class CSEChat  {
  ...
  _parsePresence(stanza:Element) {
    ...
    const cseflags = stanza.getChild('cseflags');
    const isCSE = cseflags ? cseflags.attrs.cse : senderName.slice(0,3) == 'cse'; // Kludge since presence stanzas don't currently set cseflags
