...
if (!window['cse-ui']) window['cse-ui'] = {};
window['cse-ui'].slashCommandRegistry = [];
...
export function registerSlashCommand(command: string, helpText: string, callback: (args: string) => void) {
  ...
  let found = false;
  for (let i = 0; i < window['cse-ui'].slashCommandRegistry.length; ++i) {
    if (window['cse-ui'].slashCommandRegistry[i].command === cmd) {
      found = true;
      break;
    }
  }
  if (!found) window['cse-ui'].slashCommandRegistry.push({ helpText, command: cmd });
...
export function unregisterSlashCommand(command: string) {
  ...
  if (index >= 0) window['cse-ui'].slashCommandRegistry.slice(index, 1);
}
...
export function parseMessageForSlashCommand(command: string): boolean {
  ...
  let found = false;
  for (let i = 0; i < window['cse-ui'].slashCommandRegistry.length; ++i) {
    if (window['cse-ui'].slashCommandRegistry[i].command === cmd.toLowerCase()) {
...
export function getSlashCommands(): SlashCommand[] {
  return window['cse-ui'].slashCommandRegistry.slice(0);
}
