...
export function startTimer(timerDurationInMs: number, intervalInMs: number, onUpdate: (elapsedMs: number, timeLeftInMs: number) => void) {
...
function timerTick(timerRef: TimerRef, startTime: number, intervalInMs: number, timerDurationInMs: number, onUpdate: (elapsedMs: number, timeLeftInMs: number) => void) {
    if (!timerRef.run) {
        return;
    }
