...
export function getPatcherEventName(apiHostName: string, eventName: string) {
  if (apiHostName === camelotunchained.game.signalRHost()) {
    return eventName;
  }
...
export function createPatcherHub(opts?: { hostName?: string, isMainPatcherHub?: boolean }): SignalRHub {
  const hostName = opts.hostName || camelotunchained.game.signalRHost();
...
export default function() {
  if (!camelotunchained.game.signalRHost()) return;
