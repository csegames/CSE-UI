...
export const FULL_SCREEN_HEIGHT_ID = 'Full Screen  Resolution Height';    // game is broken, has 2 spaces
...
function processQueue() {
  if (game.debug) console.log(`currentTask ${JSON.stringify(currentTask)} queue ${JSON.stringify(configQueue)}`);
      ...
      if (isNotgame()) {
        switch (config.type) {
          case ConfigIndex.AUDIO:
            if (game.debug) console.warn('emulating audio settings');
        ...
        switch (config.type) {
          ...
          case ConfigIndex.KEYBIND:
            // New Keybind API.
            if (game.debug) console.log(`type ${config.type} request all key binds`);
            // game.RequestAllKeybinds();
            break;
          case ConfigIndex.KEYBIND_CHANGED:
            // NO-OP, this is requested just prior to a call to StartRecordingKeybind
            break;
          default:
            if (game.debug) console.log(`type ${config.type} config vars`);
            // game.GetConfigVars(config.type as any);
            break;
        }
      }
    } else {
      if (game.debug) console.log('current config task is disabled, process queue');
...
// game.OnReceiveConfigVars(onReceiveConfigVars);
...
// }
// game.OnDisplayModesChanged(onReceiveDisplayModes);
...
// }
// game.OnKeybindRecorded(onKeyBindRecorded);
...
// }
// game.OnAllKeybindsRequested(onRequestAllKeybinds);

export function getKeyBinds(onconfig: (configs: any, type: ConfigIndex) => void) {
  if (game.debug) console.log('get keybinds');
...
export function recordKeybind(button: number, alias: number, onconfig: (keybind: Binding, type: ConfigIndex) => void) {
  ...
  // game.StartRecordingKeybind(button, alias);
  if (game.debug) console.log('did start recording keybind (game game is listening)');
}

export function stopRecordingKeybind() {
  // game.CancelRecordingKeybind();
  if (game.debug) console.log('did stop recording keybind (not listening now)');
  cancel(ConfigIndex.KEYBIND_CHANGED);
}

export function restoreDefaultKeybinds() {
  if (game.debug) console.log('restore default keybind settings');
...
export function sendConfigVarChangeMessage(id: string, value: string | number) {
  ...
  // window['cu'].configVarAlertMessage =
  //  window.setTimeout(() => sendSystemMessage(`${id} has been changed to ${value}`), 200);
}
