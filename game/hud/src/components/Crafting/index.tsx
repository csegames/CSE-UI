...
class Crafting extends React.Component<CraftingProps, CraftingState> {
  ...
  public render() {
    ...
    this.state.commandList.forEach((tCmd: any): void => {
      const tParam: JSX.Element = tCmd.hasOwnProperty('params') ?
        <span className='cmdItem-params'>{tCmd.params}</span> : <span></span>;
          ...
          <div className='cu-window'>
            ...
            <div className='cu-window-content'>
              ...
              <input
                type='text'
                id='input-crCmd'
                value={this.state.activeCommand || '/cr '}
                onChange={this.onCommandChanged}
                onFocus={this.onCommandFocus}
                onBlur={this.onCommandBlur}>
              </input>
  ...
  private onCommandFocus = (event: any): void => {
    client.RequestInputOwnership();
  }

  private onCommandBlur = (event: any): void => {
    client.ReleaseInputOwnership();
  }

  private onCommandChanged = (event: any): void => {
  ...
  private getCommands = (): any => {
    ...
    crCommands.push({
      ...
      desc: 'Will process a nearby substance to <qualtiy (%)>. Requires 1 substance nearby, will randomly select 1 ' +
       'substance if multiple substances are nearby. Substance must be within 5m of vox and within 2m of you.',
    ...
    crCommands.push({
      ...
      desc: 'Will take all nearby substances and shape them into an alloy. Substances must be within 5m of vox and within ' +
       '2m of you.',
      ex: '/cr shape "Iron" do',
    });
    ...
    crCommands.push({
      ...
      desc: 'Makes the <item name> item, you may optionally specify a [given name]. use /cr list weapons to view ' +
    ...
    crCommands.push({
      ...
      desc: 'Sets the type of crafting job for your vox. If there is a current job it must be cleared before ' +
       'setting a new one',
      ex: '/cr vox setjob',
    });
    ...
    crCommands.push({
      ...
      desc: 'Lists integredents from your inventory in a numbered list.  This number is used to add the ingredient ' +
       'to the vox',
    });
