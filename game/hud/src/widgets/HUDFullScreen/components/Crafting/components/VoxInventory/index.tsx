...
import { DrawerSlotNumberToItem, ContainerIdToDrawerInfo } from '../../../ItemShared/InventoryBase';
import { getItemUnitCount, firstAvailableSlot, getItemWithNewContainerPosition } from '../../../../lib/utils';
  ...
  private initializeContextState = (graphql: GraphQLResult<VoxInventoryQuery.Query>) => {
    ...
    voxItem.containerDrawers[0].containedItems.forEach((item: InventoryItem.Fragment) => {
      ...
      if (currentPosition === -1 || slotNumberToItem[currentPosition]) {
        ...
        webAPI.ItemAPI.MoveItems(webAPI.defaultConfig, game.shardID, game.selfPlayerState.characterID, moveReq as any);
        return;
      } else {
