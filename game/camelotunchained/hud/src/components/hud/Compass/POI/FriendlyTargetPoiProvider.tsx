...
> {
  ...
  public componentDidMount() {
    this.eventHandles.push(camelotunchained.game.friendlyTargetState.onUpdated(this.onSelfPlayerStateUpdated));
    this.eventHandles.push(camelotunchained.game.selfPlayerState.onUpdated(this.onFriendlyTargetStateUpdated));
  }
  ...
  private onSelfPlayerStateUpdated = () => {
    if (camelotunchained.game.friendlyTargetState.isAlive) {
      const poi = this.getFriendlyTargetPOI(camelotunchained.game.friendlyTargetState);
  ...
  private onFriendlyTargetStateUpdated = () => {
    this.setState((prevState: FriendlyTargetPoiProviderState) => {
      if (prevState.playerId !== camelotunchained.game.selfPlayerState.characterID) {
        return {
          playerId: camelotunchained.game.selfPlayerState.characterID,
        };
      } else {
        return null;
      }
