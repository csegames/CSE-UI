...
export function parseMarkdown(text: string, opts: OptionsState, next: Parser): [string, JSX.Element] {
  ...
  return [
    ...
    (
      <>
        {
          isItalic ?
            <i key={genID()}>{next(content, opts, next)}</i> :
            <b key={genID()}>{next(content, opts, next)}</b>
        }
