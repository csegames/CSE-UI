...
export class SiegeHealth extends React.Component<SiegeHealthProps, SiegeHealthState> {
    ...
    switch (this.props.for) {
      case HealthFor.Self: {
        this.eventHandles.push(camelotunchained.game.selfPlayerState.onUpdated(() => {
          this.setState({ entity: cloneDeep(camelotunchained.game.selfPlayerState) });
        }));
        break;
      }
      ...
      case HealthFor.EnemyTarget: {
        this.eventHandles.push(camelotunchained.game.enemyTargetState.onUpdated(() => {
          this.setState({ entity: cloneDeep(camelotunchained.game.enemyTargetState) });
        }));
        break;
      }
      ...
      case HealthFor.FriendlyTarget: {
        this.eventHandles.push(camelotunchained.game.friendlyTargetState.onUpdated(() => {
          this.setState({ entity: cloneDeep(camelotunchained.game.friendlyTargetState) });
        }));
        break;
      }
    }
  ...
  public shouldComponentUpdate(nextProps: SiegeHealthProps, nextState: SiegeHealthState) {
    ...
    switch (next.type) {
      case 'player': {
        ...
        if (!camelotunchained.game.entities) return false;

        const currentControlledEntity = camelotunchained.game.entities[thisControlled];
        const nextControlledEntity = camelotunchained.game.entities[nextControlled];
  ...
  public render() {
    ...
    switch (this.state.entity.type) {
      case 'player': {
        const controlled = camelotunchained.game.entities &&
          camelotunchained.game.entities[this.state.entity.controlledEntityID];
