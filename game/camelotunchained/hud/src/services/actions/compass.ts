...
export function getCompassData() {
  if (camelotunchained.game.selfPlayerState.isReady) {
    ...
    const x = Math.round(camelotunchained.game.selfPlayerState.position.x);
    const y = Math.round(camelotunchained.game.selfPlayerState.position.y);
    const z = Math.round(camelotunchained.game.selfPlayerState.position.z);
    return {
      facing,
      facingNorth,
      position: {
...
export function getCompassPositionData() {
  if (camelotunchained.game.selfPlayerState.isReady && camelotunchained.game.selfPlayerState.position) {
    const x = Math.round(camelotunchained.game.selfPlayerState.position.x);
    const y = Math.round(camelotunchained.game.selfPlayerState.position.y);
    const z = Math.round(camelotunchained.game.selfPlayerState.position.z);
    return {
